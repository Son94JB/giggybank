...
class TransactionHistoryFragment : BaseFragment<FragmentTransactionHistoryBinding>(
    ...
    private var startDate = StringFormatUtil.dateToString(0)
        ...
        with(binding){
            ...
            fragmentTransactionHistorySpinner.setOnItemSelectedListener { view, position, id, item ->
                ...
                when(item){
                    ...
                    "식비 내역" ->{
                        adapter.submitList(transactionHistoryFragmentViewModel.transactionList.value?.filter { it.category == "식비" })
                    }
                    "교통 내역" ->{
